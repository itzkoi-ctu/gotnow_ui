...
const ManageOrder = () => {
  ...
  const handleUpdateStatus = async (orderId, currentStatus) => {
    event.stopPropagation(); // Ngăn sự kiện mở OrderAdminDetail khi bấm nút cập nhật
              ...
              <table className="table table-hover">
                ...
                <tbody>
                  {filteredOrders.map((order) => (
                    <tr onClick={() => handleOpenDetail(order.id)} style={{ cursor: "pointer" }}>
                    <td>{order.id}</td>
                    ...
                    <td onClick={(e) => e.stopPropagation()}> {/* Ngăn sự kiện click từ nút lan lên hàng */}
                      {nextStatus[order.orderStatus] && (
                        <button
                        className="btn btn-sm btn-outline-primary"
                        onClick={(event) => handleUpdateStatus(order.id, order.orderStatus, event)}
